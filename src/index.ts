...
const dslPath = path.resolve("./sample/SSDT-8.dsl")
/**
...
function abort(tag:string, msg:string = null) {
    if (msg != null) {
        tag = `${chalk.green(`[${tag}]`)} ${chalk.red(msg)}`
    } else {
        tag = chalk.red(msg)
    }
    console.error(tag)
...
async function main() {
    ...
    let DSDT = await fs.readFile(filePath, "utf8")
    const ECF = DSDT.match(/OperationRegion \((?<id>[0-9A-F]{4}), EmbeddedControl, Zero, 0xFF\)/i)?.[1]
    if (ECF == null) {
    ...
    for (const fields of fieldChunk) {
        ...
        for (const field of fieldMap) {
            ...
            const fieldEle:SingleFieldElement = {
            ...
            if (fieldName.trim().length >= 3 && fieldValue > 8) {
                ...
                const regex1 = new RegExp(
                    `(${SCOPE.replace(/\\/g, "\\\\").replace(/\./g, "\\.")}\\.)${fieldName}([\\),\\s]|$)`,
                    "gm"
                )
                DSDT = DSDT.replace(regex1, (match:string) => {
                ...
                DSDT = DSDT.replace(regex2, (match: string) => {
        ...
        for (const field of fieldArray) {
            if (field.type === FieldType.PREPOSTFIX) {
                ...
                fieldConstruct.push(`${field.padding}${
                    field.original.fieldName.padStart(4)
                },${
                    field.original.fieldSize.toString().padStart(4)
                }, // 0x${offsetHex}, ${field.original.bitOffset}bit`)
                ...
                for (const subField of field.replaced) {
                    ...
                    fieldConstruct.push(`${field.padding}${
                        subField.fieldName.padStart(4)
                    },${
                        subField.fieldSize.toString().padStart(4)
                    }, // ${field.original.fieldName.padStart(4)}, 0x${offsetHex} , ${subField.bitOffset}bit`)
                }
                const offsetHex = field.original.byteOffset.toString(16).toUpperCase().padStart(2, "0")
                ...
                if (roundedSize <= 4) {
                    DSDT = DSDT.replace(
                        new RegExp(`\\$RP#IDSIMPLE#${field.original.fieldName}#`, "g"),
                        `B1B${roundedSize}(${subNames}) /* ${field.original.fieldName} */`)
                    DSDT = DSDT.replace(
                        new RegExp(`\\$RP#IDABSOLUTE#${field.original.fieldName}#`, "g"),
                        `B1B${roundedSize}(${subAbsoluteNames}) /* ${field.original.fieldName} */`)
                } else {
                    DSDT = DSDT.replace(
                        new RegExp(`\\$RP#IDSIMPLE#${field.original.fieldName}#`, "g"),
                        `RECB(0x${offsetHex},${field.original.fieldSize}) /* ${field.original.fieldName} */`)
                    DSDT = DSDT.replace(
                        new RegExp(`\\$RP#IDABSOLUTE#${field.original.fieldName}#`, "g"),
                        `RECB(0x${offsetHex},${field.original.fieldSize}) /* ${field.original.fieldName} */`)
                }
    ...
    const exportPath = path.resolve(path.dirname(filePath), `patched_${path.basename(filePath)}`)
    await fs.writeFile(exportPath, DSDT, {encoding: "utf8"})
...
interface IncPadding {
    type:FieldType
    padding:string
}

interface RawFieldElement extends IncPadding {
    type:FieldType.PREPOSTFIX,
    raw:string
}
interface OffsetFieldElement extends IncPadding {
    type:FieldType.OFFSET,
    byteOffset:number
}
interface SingleFieldElement extends IncPadding {
    type:FieldType.SINGLEFIELD | FieldType.MULTIFIELD,
    original:SingleField,
    isLarge:boolean,
    replaced?: SingleField[],
}
type FieldElement = RawFieldElement | OffsetFieldElement | SingleFieldElement
